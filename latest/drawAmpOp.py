...
class ampOp(inkBase.inkscapeMadeEasy):
    ...
    # ---------------------------------------------
    def drawOpAmpGeneral(self, parent, position=[0, 0], label='OpAmp', mirrorInput=False, drawVin=False, drawIin=False,
                         drawVd=False, drawVout=False, drawIout=False, inputVPosName='v^+', inputVNegName='v^-',
                         inputIPosName='i^+', inputINegName='i^-', VoutName='v_o', IoutName='i_o', VdiffName='v_d',
                         flagDrawSupply=False, FlagSupplyValues=False, flagSupplySymm=True, supplyPositiveVal='V_{cc}',
        ...
        if flagDrawSupply:
            ...
            if FlagSupplyValues:
                if flagSupplySymm:
                    self.drawSignal(elem, [position[0] + 17.5, position[1] - 25], angleDeg=0,
                               nodalVal='+' + supplyPositiveVal.replace('+', ''))
                    self.drawSignal(elem, [position[0] + 17.5, position[1] + 25], angleDeg=180,
                               nodalVal='-' + supplyPositiveVal.replace('+', ''))
                else:
                    self.drawSignal(elem, [position[0] + 17.5, position[1] - 25], angleDeg=0,
                               nodalVal='+' + supplyPositiveVal.replace('+', ''))
        ...
        if drawVin:
            ...
            temp1 = inkDraw.text.latex(self, group, textVTop, posTop, textColor=self.voltageColor,
                                       fontSize=self.fontSizeSmall, refPoint='br', preambleFile=self.preambleFile)
            temp2 = inkDraw.text.latex(self, group, textVBot, posBot, textColor=self.voltageColor,
                                       fontSize=self.fontSizeSmall, refPoint='br', preambleFile=self.preambleFile)
        ...
        if drawIin:
            ...
            temp1 = self.drawCurrArrow(group, posTop, name=textITop, color=self.currentColor, angleDeg=180,
                                       invertArrows=False)
            self.rotateElement(temp1, posTop, 180)
            temp1 = self.drawCurrArrow(group, posBot, name=textIBot, color=self.currentColor, angleDeg=180,
                                       invertArrows=False)
        ...
        if drawVout:
            ...
            temp1 = inkDraw.text.latex(self, group, VoutName, postext, textColor=self.voltageColor,
                                       fontSize=self.fontSize, refPoint='bl', preambleFile=self.preambleFile)

        # write i_out
        if drawIout:
            pos = [position[0] + 42, position[1] + 3]

            temp1 = self.drawCurrArrow(group, pos, name=IoutName, color=self.currentColor, angleDeg=180,
                                       invertArrows=False)
            self.rotateElement(temp1, pos, 180)

        if drawVd:
            pos1 = [position[0] + 6, position[1]]
            temp1 = self.drawVoltArrow(group, pos1, name=VdiffName, color=self.voltageColor, angleDeg=90,
                                       invertArrows=not mirrorInput, size=15)
