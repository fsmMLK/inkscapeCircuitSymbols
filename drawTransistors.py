...
class transistor(inkBase.inkscapeMadeEasy):
    ...
    def drawTransistorBJT(self, parent, position=[0, 0], angleDeg=0, label='BJT', mirrorEC=False, drawBCEtags=False,
                          drawEnvelope=False, transistorType='NPN', flagPhototransistor=False, drawVCE=False,
                          drawVCB=False, drawVBE=False, drawICarrow=False, drawIBarrow=False, drawIEarrow=False,
                          VCEname='V_{ce}', VCBname='V_{cb}', VBEname='V_{be}', ICname='i_c', IBname='i_b',
                          IEname='i_e'):
        ...
        L_arrow = 2.5
        markerBJT = inkDraw.marker.createMarker(self, 'BJTArrow',
                                                'M 0,0 l -%f,%f l 0,-%f z' % (L_arrow * 1.2, L_arrow / 2.0, L_arrow),
                                                RenameMode=1, strokeColor=colorBlack, fillColor=colorBlack,
                                                lineWidth=0.6)
        ...
        if mirrorEC:
            if isNPN:
                ...
                inkDraw.line.relCoords(elem, [[7, -5]], [position[0] + 18, position[1] - 3],
                                       lineStyle=lineStyleArrow)  # emitter arrow
                inkDraw.line.relCoords(elem, [[0, -17]], [position[0] + 25, position[1] - 8])  # collector
            else:
                inkDraw.line.relCoords(elem, [[7, 5], [0, 17]], [position[0] + 18, position[1] + 3])  # emitter
                inkDraw.line.relCoords(elem, [[-7, 5]], [position[0] + 25, position[1] - 8],
                                       lineStyle=lineStyleArrow)  # emitter arrow
                inkDraw.line.relCoords(elem, [[0, -17]], [position[0] + 25, position[1] - 8])  # collector
        ...
        if drawEnvelope:
            inkDraw.circle.centerRadius(elem, centerPoint=[position[0] + 22, position[1]], radius=10, offset=[0, 0],
                                        label='circle')

        if drawBCEtags:
            if not flagPhototransistor:
                tB = inkDraw.text.latex(self, group, 'B', position=[position[0] + 10, position[1] - 3],
                                        fontSize=self.fontSizeSmall / 1.5, refPoint='cc',
                                        preambleFile=self.preambleFile, angleDeg=-angleDeg)
            tC = inkDraw.text.latex(self, group, 'C', position=pos_Ctag, fontSize=self.fontSizeSmall / 1.5,
                                    refPoint='cc', preambleFile=self.preambleFile, angleDeg=-angleDeg)
            tE = inkDraw.text.latex(self, group, 'E', position=pos_Etag, fontSize=self.fontSizeSmall / 1.5,
                                    refPoint='cc', preambleFile=self.preambleFile, angleDeg=-angleDeg)
        ...
        if drawVCE:
            ...
            self.drawVoltArrowSimple(group, pos, name=VCEname, color=self.voltageColor, angleDeg=90,
                                     invertArrows=mirrorEC, size=20.0, invertCurvatureDirection=False,
        ...
        if drawVCB:
            ...
            self.drawVoltArrowSimple(group, pos, name=VCBname, color=self.voltageColor, angleDeg=ang,
                                     invertArrows=False, size=20.0, invertCurvatureDirection=not mirrorEC,
                                     extraAngleText=angleDeg)
        ...
        if drawVBE:
            ...
            self.drawVoltArrowSimple(group, pos, name=VBEname, color=self.voltageColor, angleDeg=ang, invertArrows=True,
                                     size=20.0, invertCurvatureDirection=mirrorEC, extraAngleText=angleDeg)

        # draw terminal currents
        if drawICarrow:
            if mirrorEC:
                self.drawCurrArrowSimple(group, [position[0] + 30, position[1] + 17.5], name=ICname,
                                         color=self.currentColor, angleDeg=90, invertArrows=not isNPN, size=7.5,
                                         invertTextSide=True, extraAngleText=angleDeg)
            else:
                self.drawCurrArrowSimple(group, [position[0] + 30, position[1] - 17.5], name=ICname,
                                         color=self.currentColor, angleDeg=90, invertArrows=isNPN, size=7.5,
                                         invertTextSide=True, extraAngleText=angleDeg)

        if drawIBarrow:
            self.drawCurrArrowSimple(group, [position[0] + 7.5 - 10, position[1] - 5], name=IBname,
                                     color=self.currentColor, angleDeg=0, invertArrows=not isNPN, size=7.5,
                                     invertTextSide=False, extraAngleText=angleDeg)

        if drawIEarrow:
            if mirrorEC:
                self.drawCurrArrowSimple(group, [position[0] + 30, position[1] - 17.5], name=IEname,
                                         color=self.currentColor, angleDeg=90, invertArrows=not isNPN, size=7.5,
                                         invertTextSide=True, extraAngleText=angleDeg)
            else:
                self.drawCurrArrowSimple(group, [position[0] + 30, position[1] + 17.5], name=IEname,
    ...
    def drawTransistorMOSFET(self, parent, position=[0, 0], angleDeg=0, label='MOSFET', mirrorSD=False,
                             drawSGDtags=False, drawEnvelope=False, modeType='MOSFET-E', gateType='P_gate',
                             bodyDiode=False, drawVGS=False, drawVDS=False, drawVDG=False, drawIDarrow=False,
                             drawISarrow=False, drawIGarrow=False, VGSname='V_{GS}', VDSname='V_{SD}', VDGname='V_{GD}',
        ...
        L_arrow = 2.0
        markerMOS = inkDraw.marker.createMarker(self, 'MOSArrow',
                                                'M -0.3,0 l -%f,%f l 0,-%f z' % (L_arrow * 1.2, L_arrow / 2.0, L_arrow),
                                                RenameMode=1, strokeColor=colorBlack, fillColor=colorBlack,
                                                lineWidth=0.6)
        ...
        if mirrorSD:
            ...
            inkDraw.line.relCoords(elem, [[5, 0], [0, 5.25]], [position[0] + 19, position[1] + 5.25],
                                   lineStyle=lineStyleFine)  # drain line

            inkDraw.line.relCoords(elem, [[0, -20.75]], [position[0] + 24, position[1] - 5.25])  # source line
            inkDraw.line.relCoords(elem, [[5, 0], [0, 5.25]], [position[0] + 19, position[1] - 5.25],
                                   lineStyle=lineStyleFine)  # source line

            inkDraw.circle.centerRadius(elem, [position[0] + 24, position[1] - 5.25], radius=0.4, offset=[0, 0],
                                        label='circle')  # source dot
            if isNgate:
                inkDraw.line.relCoords(elem, [[-5, 0]], [position[0] + 24, position[1] + 0],
                                       lineStyle=lineStyleArrow)  # horizontal arrow line
            else:
                inkDraw.line.relCoords(elem, [[5, 0]], [position[0] + 19, position[1] + 0],
                                       lineStyle=lineStyleArrow)  # horizontal arrow line

            if bodyDiode and isEmode:
                inkDraw.circle.centerRadius(elem, [position[0] + 24, position[1] + 5.25], radius=0.4, offset=[0, 0],
                                            label='circle')  # diode cathode dot
                inkDraw.line.relCoords(elem, [[4, 0], [0, -3.75]], [position[0] + 24, position[1] + 5.25],
                                       lineStyle=lineStyleFine)  # diode cathode
                inkDraw.line.relCoords(elem, [[4, 0], [0, 3.75]], [position[0] + 24, position[1] - 5.25],
                                       lineStyle=lineStyleFine)  # diode anode

                if isNgate:
                    inkDraw.line.relCoords(elem, [[3, 0]], [position[0] + 26.5, position[1] + 1.5],
                                           lineStyle=lineStyleFine)  # diode cathode side line
                    inkDraw.line.relCoords(elem, [[3, 0], [-1.5, 3], [-1.5, -3]],
                                           [position[0] + 26.5, position[1] - 1.5], lineStyle=lineStyleFine)  # diode
                else:
                    inkDraw.line.relCoords(elem, [[3, 0]], [position[0] + 26.5, position[1] - 1.5],
                                           lineStyle=lineStyleFine)  # diode cathode side line
                    inkDraw.line.relCoords(elem, [[3, 0], [-1.5, -3], [-1.5, 3]],
        ...
        if isEmode:
            ...
            inkDraw.line.relCoords(elem, [[0, 3.5]], [position[0] + 19, position[1] - 7],
                                   lineStyle=lineStyleFine)  # vertical gate line
            inkDraw.line.relCoords(elem, [[0, 3.5]], [position[0] + 19, position[1] - 1.75],
                                   lineStyle=lineStyleFine)  # vertical gate line
            inkDraw.line.relCoords(elem, [[0, 3.5]], [position[0] + 19, position[1] + 3.5],
                                   lineStyle=lineStyleFine)  # vertical gate line
        else:
            inkDraw.line.relCoords(elem, [[0, 14]], [position[0] + 19, position[1] - 7],
                                   lineStyle=lineStyleFine)  # vertical gate line
        ...
        if drawEnvelope:
            if bodyDiode and isEmode:
                ...
                inkDraw.circle.centerRadius(elem, centerPoint=[position[0] + 20, position[1]], radius=10, offset=[0, 0],
                                            label='circle')

        if drawSGDtags:
            tB = inkDraw.text.latex(self, group, 'G', position=pos_Gtag, fontSize=self.fontSizeSmall / 1.5,
                                    refPoint='cc', preambleFile=self.preambleFile, angleDeg=-angleDeg)
            tC = inkDraw.text.latex(self, group, 'D', position=pos_Dtag, fontSize=self.fontSizeSmall / 1.5,
                                    refPoint='cc', preambleFile=self.preambleFile, angleDeg=-angleDeg)
            tE = inkDraw.text.latex(self, group, 'S', position=pos_Stag, fontSize=self.fontSizeSmall / 1.5,
                                    refPoint='cc', preambleFile=self.preambleFile, angleDeg=-angleDeg)
        ...
        if drawVDS:
            ...
            self.drawVoltArrowSimple(group, pos, name=VDSname, color=self.voltageColor, angleDeg=90,
                                     invertArrows=mirrorSD, size=20.0, invertCurvatureDirection=False,
                                     extraAngleText=angleDeg)
        ...
        if drawVGS:
            if mirrorSD:
                ...
                pos = [position[0] + 15, position[1] + 14]
                ang = -19
            self.drawVoltArrowSimple(group, pos, name=VGSname, color=self.voltageColor, angleDeg=ang, invertArrows=True,
                                     size=10.0, invertCurvatureDirection=mirrorSD, extraAngleText=angleDeg)
        ...
        if drawVDG:
            ...
            self.drawVoltArrowSimple(group, pos, name=VDGname, color=self.voltageColor, angleDeg=ang,
                                     invertArrows=False, size=20.0, invertCurvatureDirection=not mirrorSD,
                                     extraAngleText=angleDeg)
        ...
        if drawISarrow:
            ...
            self.drawCurrArrowSimple(group, pos, name=ISname, color=self.currentColor, angleDeg=90,
                                     invertArrows=not mirrorSD, size=7.5, invertTextSide=True, extraAngleText=angleDeg)
        ...
        if drawIGarrow:
            ...
            self.drawCurrArrowSimple(group, pos, name=IGname, color=self.currentColor, angleDeg=0, invertArrows=False,
                                     size=7.5, invertTextSide=not mirrorSD, extraAngleText=angleDeg)
        ...
        if drawIDarrow:
            if mirrorSD:
                ...
                pos = [position[0] + 29, position[1] - 17.5]
            self.drawCurrArrowSimple(group, pos, name=IDname, color=self.currentColor, angleDeg=90,
                                     invertArrows=not mirrorSD, size=7.5, invertTextSide=True, extraAngleText=angleDeg)
    ...
    def drawTransistorJFET(self, parent, position=[0, 0], angleDeg=0, label='JFET', mirrorSD=False, drawSGDtags=False,
                           drawEnvelope=False, gateType='P_gate', moveGate=False, drawVGS=False, drawVDS=False,
                           drawVDG=False, drawIDarrow=False, drawISarrow=False, drawIGarrow=False, VGSname='V_{GS}',
                           VDSname='V_{SD}', VDGname='V_{GD}', IDname='i_d', ISname='i_s', IGname='i_g'):
        ...
        if drawEnvelope:
            inkDraw.circle.centerRadius(elem, centerPoint=[position[0] + 19, position[1]], radius=10, offset=[0, 0],
                                        label='circle')
        ...
        if drawVGS:
            if mirrorSD:
                ...
                if moveGate:
                    ...
                    L = 15
            self.drawVoltArrowSimple(group, pos, name=VGSname, color=self.voltageColor, angleDeg=ang, invertArrows=True,
                                     size=L, invertCurvatureDirection=mirrorSD, extraAngleText=angleDeg)
        ...
        if drawVDG:
            if mirrorSD:
                ...
                if moveGate:
                    ...
                    L = 15
            self.drawVoltArrowSimple(group, pos, name=VDGname, color=self.voltageColor, angleDeg=ang,
                                     invertArrows=False, size=L, invertCurvatureDirection=not mirrorSD,
                                     extraAngleText=angleDeg)
